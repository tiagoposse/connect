...
func (h *OgentHandler) CreateApiKey(ctx context.Context, req *CreateApiKeyReq) (CreateApiKeyRes, error) {
	...
	b.SetName(req.Name)
	b.SetKey(req.Key)
	b.SetScopes(*JsonConvert(req.Scopes, &authorization.Scopes{}).(*authorization.Scopes))
...
func (h *OgentHandler) ListAudit(ctx context.Context, params ListAuditParams) (ListAuditRes, error) {
	q := h.client.Audit.Query().WithUser()
	page := 1
	if v, ok := params.XPage.Get(); ok {
		page = v
	}
	...
	r := NewAuditLists(es)
	countQuery, err := h.client.Audit.Query().WithUser().Count(ctx)
	if err != nil {
		switch {
		case ent.IsNotFound(err):
			return &R404{
...
func (h *OgentHandler) ReadAudit(ctx context.Context, params ReadAuditParams) (ReadAuditRes, error) {
	q := h.client.Audit.Query().Where(audit.IDEQ(params.ID)).WithUser()
	e, err := q.Only(ctx)
	if err != nil {
		switch {
		case ent.IsNotFound(err):
...
func (h *OgentHandler) UpdateGroup(ctx context.Context, req *UpdateGroupReq, params UpdateGroupParams) (UpdateGroupRes, error) {
	...
	if req.Scopes != nil {
		v := req.Scopes
		b.SetScopes(*JsonConvert(v, &authorization.Scopes{}).(*authorization.Scopes))
	}
