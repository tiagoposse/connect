...
import (
	...
	ogent_auth "github.com/tiagoposse/ogent-auth/authentication"
	ogentauthext "github.com/tiagoposse/ogent-auth/extension"
)

func main() {	
	spec := new(ogen.Spec)

	authzExt := ogentauthext.NewOgentAuthExtension(
		ogentauthext.WithApiKeySecurity(),
		ogentauthext.WithCookieSecurity(),
		ogentauthext.WithCustomOperation("status", []authentication.SecuritySchemeType{ogent_auth.SchemeApiKey, ogent_auth.SchemeCookie}, authz.Scopes{}),
		ogentauthext.WithCustomOperation("logout", []authentication.SecuritySchemeType{ogent_auth.SchemeApiKey, ogent_auth.SchemeCookie}, authz.Scopes{}),
		ogentauthext.WithCustomOperation("googleAuthSync", []authentication.SecuritySchemeType{ogent_auth.SchemeApiKey, ogent_auth.SchemeCookie},
			authz.Scopes{types.AdminAll, types.AdminSSOAll, types.AdminSSOSync},
		),
		ogentauthext.WithDefaultGlobalAuthentication(),
	)
	...
	}, entc.Extensions(authzExt, ogent, oas, listExt))
	if err != nil {
