...
func (uq *UserQuery) QueryKeys() *ApiKeyQuery {
...
func (uq *UserQuery) WithKeys(opts ...func(*ApiKeyQuery)) *UserQuery {
	query := (&ApiKeyClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withKeys; query != nil {
		if err := uq.loadKeys(ctx, query, nodes,
			func(n *User) { n.Edges.Keys = []*ApiKey{} },
			func(n *User, e *ApiKey) { n.Edges.Keys = append(n.Edges.Keys, e) }); err != nil {
			return nil, err
		}
	}
	...
	for name, query := range uq.withNamedKeys {
		if err := uq.loadKeys(ctx, query, nodes,
...
// edge with the given name. The optional arguments are used to configure the query builder of the edge.
func (uq *UserQuery) WithNamedKeys(name string, opts ...func(*ApiKeyQuery)) *UserQuery {
	query := (&ApiKeyClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	if uq.withNamedKeys == nil {
